...
// 
window.quickTools = {}
const checkingInterval = 500 // 500 milliseconds 
const loopingFunctions = []
setInterval(() => {
        // call each of the looping functions
        for (let each of loopingFunctions) {
            each()
        }
    }, checkingInterval)
...
window.css = (strings, ...values) => {
        ...
        // 
        let output = ""
        for (let eachIndex in strings) {
            let [eachString, eachValue] = [ strings[eachIndex], values[eachIndex] ]
            if (eachValue != undefined) {
                eachString += `${eachValue}`
            }
            output += eachString
        }
...
// the following class was obtained from: https://raw.githubusercontent.com/apvarun/toastify-js/master/src/toastify-es.js
/*!
 * Toastify js 1.11.1
...
class Toastify {
    ...
    _buildToast() {
        ...
        if (this.options.close === true) {
            ...
            if (this.options.position == "left" && width > toastStyle.centerallThreshold) {
                // Adding close icon on the left of content
    ...
    _reposition() {
        ...
        let topLeftOffsetSize = {
            top: toastStyle.padding,
            bottom: toastStyle.padding,
        }
        ...
        for (let i = 0; i < allToasts.length; i++) {
            ...
            let offset = toastStyle.offset

            let width = window.innerWidth > 0 ? window.innerWidth : screen.width

            // Show toast in center if screen with less than or equal to 360px
            if (width <= toastStyle.centerallThreshold) {
...
window.quickTools.Toastify = Toastify
// css for toastify
css`
    .toastify {
        padding: 12px 20px;
    ...
    .toastify-right {
        right: ${toastStyle.padding}px;
    }

    .toastify-left {
        left: ${toastStyle.padding}px;
    }
    ...
    @media only screen and (max-width: ${toastStyle.centerallThreshold}px) {
...
// get data
loopingFunctions.push(async ()=>{
    // update the data variable
    window[Symbol.for("visualizerData")] = await get(`${window.location.origin}/data`)
})
...
new window.quickTools.Toastify({
